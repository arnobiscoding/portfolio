...
export default function ContactSection() {
  ...
  return (
    <section
      ...
      className={`min-h-[60vh] flex flex-col items-center justify-center py-20 mt-32 transition-opacity transition-transform duration-700 ease-out
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-12 w-full max-w-4xl px-4 mb-8" role="list" aria-label="Contact Methods">
        {contacts.map((contact) => (
          <a
            ...
            role="listitem"
            className="flex flex-col items-center justify-center bg-card border border-border rounded-xl shadow-md p-8 hover:scale-105 hover:bg-secondary hover:shadow-lg transition-transform duration-300 group min-h-[170px]"
            aria-label={contact.label}
          >
            <span className="mb-4 text-primary transition-colors">{contact.icon}</span>
            <span className="text-lg font-medium transition-colors">{contact.label}</span>
          </a>
        ))}
      </div>
      <p className="text-muted-foreground mt-6">Feel free to reach out via any platform!</p>
    </section>
  );
} 